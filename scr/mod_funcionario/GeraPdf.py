...
class PDF(FPDF):
    ...
    def listaTodos(self):
        ...
        pdf.cell(col_width, row_height, 'Matrícula', 1, 0, 'L')
        pdf.cell(col_width, row_height, 'CPF', 1, 0, 'L')
        pdf.cell(col_width, row_height, 'Telefone', 1, 0, 'L')
        pdf.cell(col_width, row_height, 'Grupo', 1, 1, 'L')  # Move para a próxima linha
        ...
        try:
            ...
            for row in result[0]:
                pdf.cell(col_width, 5, str(row['id_funcionario']), 1, 0, 'L')
                pdf.cell(col_width, 5, str(row['nome']), 1, 0, 'L')
                pdf.cell(col_width, 5, str(row['matricula']), 1, 0, 'L')
                pdf.cell(col_width, 5, str(row['cpf']), 1, 0, 'L')
                pdf.cell(col_width, 5, str(row['telefone']), 1, 0, 'L')
                pdf.cell(col_width, 5, str(row['grupo']), 1, 1, 'L')  # Move para a próxima linha
